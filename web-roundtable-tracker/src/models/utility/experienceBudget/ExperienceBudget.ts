export class ExperienceBudget extends Number {
    ...
    static budgetToBaseReward = (xpBudget: ExperienceBudget, partySize: number = 4): ExperienceBudget => {
        ...
        const searchUpperBound = Math.max(400, value + Math.abs(deltaCharacters) * 120);

        let bestBaseReward = searchLowerBound;
        let bestDifference = Number.POSITIVE_INFINITY;

        for (let candidate = searchLowerBound; candidate <= searchUpperBound; candidate += 1) {
            const adjustment = ExperienceBudget.resolveCharacterAdjustmentValue(candidate);
            const adjustedBudget = candidate + deltaCharacters * adjustment;
            const difference = Math.abs(adjustedBudget - value);

            if (
                difference < bestDifference ||
                (difference === bestDifference && Math.abs(candidate - value) < Math.abs(bestBaseReward - value))
            ) {
    ...
    static budgetToCharacterAdjustment = (xpBudget: ExperienceBudget, _partySize: number = 4): ExperienceBudget => {
        return new ExperienceBudget(ExperienceBudget.resolveCharacterAdjustmentValue(xpBudget.valueOf()));
    }
