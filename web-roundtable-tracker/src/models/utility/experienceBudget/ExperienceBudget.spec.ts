...
describe('ExpreirenceBudget', () => {
	const threatLevels = [
		...
		{
			name: 'ModeratePlus',
			budget: ExperienceBudget.ModeratePlus,
			adjustment: 30,
		},
		...
		{
			name: 'ExtremePlus',
			budget: ExperienceBudget.ExtremePlus,
			adjustment: 50,
		},
		{
			name: 'Extreme2Plus',
			budget: ExperienceBudget.Extreme2Plus,
			adjustment: 50,
		},
		{
			name: 'Extreme3Plus',
			budget: ExperienceBudget.Extreme3Plus,
			adjustment: 60,
		},
	...
	it('budgetToBaseReward should preserve base reward where adjusted values are uniquely invertible', () => {
		for (let partySize = 1; partySize <= 8; partySize += 1) {
			const scenarios = threatLevels.map((threat) => ({
				...threat,
				adjustedBudget:
					threat.budget.valueOf() + (partySize - 4) * threat.adjustment,
			}));
			...
			for (const scenario of scenarios) {
				collisions.set(
					scenario.adjustedBudget,
					(collisions.get(scenario.adjustedBudget) ?? 0) + 1
				);
			}
	...
	it('budgetToBaseReward should choose the nearest matching base reward for overlapping scenarios', () => {
		expect(
			ExperienceBudget.budgetToBaseReward(new ExperienceBudget(0), 1).valueOf()
		).toBe(60);
		expect(
			ExperienceBudget.budgetToBaseReward(new ExperienceBudget(20), 2).valueOf()
		).toBe(40);
	});

	it('budgetToCharacterAdjustment should match table ranges', () => {
		expect(
			ExperienceBudget.budgetToCharacterAdjustment(
				new ExperienceBudget(20)
			).valueOf()
		).toBe(5);
		expect(
			ExperienceBudget.budgetToCharacterAdjustment(
				ExperienceBudget.Trivial
			).valueOf()
		).toBe(10);
		expect(
			ExperienceBudget.budgetToCharacterAdjustment(
				ExperienceBudget.Low
			).valueOf()
		).toBe(20);
		expect(
			ExperienceBudget.budgetToCharacterAdjustment(
				ExperienceBudget.Moderate
			).valueOf()
		).toBe(20);
		expect(
			ExperienceBudget.budgetToCharacterAdjustment(
				ExperienceBudget.ModeratePlus
			).valueOf()
		).toBe(30);
		expect(
			ExperienceBudget.budgetToCharacterAdjustment(
				ExperienceBudget.Severe
			).valueOf()
		).toBe(30);
		expect(
			ExperienceBudget.budgetToCharacterAdjustment(
				ExperienceBudget.SeverePlus
			).valueOf()
		).toBe(40);
		expect(
			ExperienceBudget.budgetToCharacterAdjustment(
				ExperienceBudget.Extreme
			).valueOf()
		).toBe(40);
		expect(
			ExperienceBudget.budgetToCharacterAdjustment(
				ExperienceBudget.ExtremePlus
			).valueOf()
		).toBe(50);
		expect(
			ExperienceBudget.budgetToCharacterAdjustment(
				ExperienceBudget.Extreme2Plus
			).valueOf()
		).toBe(50);
		expect(
			ExperienceBudget.budgetToCharacterAdjustment(
				ExperienceBudget.Extreme3Plus
			).valueOf()
		).toBe(60);
		expect(
			ExperienceBudget.budgetToCharacterAdjustment(
				ExperienceBudget.Impossible
			).valueOf()
		).toBe(60);
	});
