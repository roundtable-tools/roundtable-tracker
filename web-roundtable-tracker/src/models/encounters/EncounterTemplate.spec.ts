 ...
 */
function createTestTemplate(overrides?: Partial<EncounterTemplateData>): EncounterTemplateData {
  ...
  const defaultParticipants = hasExplicitVariants ? undefined : [createCreature()];
...
describe('EncounterTemplate Business Logic', () => {
  ...
  describe('XP Budget Calculation', () => {
    ...
    it('should apply 1/5 multiplier to simple hazards', () => {
      ...
      const simpleHazard = createHazard({ id: simpleHazardId, role: 'simple', relativeLevel: 0 });
      const complexHazard = createHazard({ id: complexHazardId, role: 'complex', relativeLevel: 0 });
      ...
      const simpleBudget = simpleInstance.calculateXpBudget(simpleVariantId).valueOf();
      const complexBudget = complexInstance.calculateXpBudget(complexVariantId).valueOf();
    ...
    it('should multiply XP by participant count', () => {
      ...
      const singleBudget = singleInstance.calculateXpBudget(singleVariantId).valueOf();
      const tripleBudget = tripleInstance.calculateXpBudget(tripleVariantId).valueOf();
  ...
  describe('Threat Level Calculation', () => {
    ...
    it('should vary threat with party size', () => {
      ...
      const smallThreat = smallInstance.calculateThreatLevel(smallVariantId).threat;
      const largeThreat = largeInstance.calculateThreatLevel(largeVariantId).threat;
