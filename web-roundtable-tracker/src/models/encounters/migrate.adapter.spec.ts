...
function validateMigration(
	...
	if (originalName !== migrated.name) {
		result.errors.push(
			`Name mismatch: expected "${originalName}", got "${migrated.name}"`
		);
	}
	...
	const totalNewParticipants = migrated.variants.reduce(
		(sum, v) => sum + v.participants.length,
		0
	);
	...
	const hasDefaultVariant = migrated.variants.some(
		(v) => v.id === migrated.defaultVariantId
	);
	...
	for (const variant of migrated.variants) {
		for (const participant of variant.participants) {
			...
			if (participant.count === undefined)
				result.errors.push('Participant missing count');

			if (participant.relativeLevel === undefined)
				result.errors.push('Participant missing relativeLevel');

			if (participant.side === undefined)
				result.errors.push('Participant missing side');

			if (participant.type === undefined)
				result.errors.push('Participant missing type');
			...
			if (
				...
				participant.successesToDisable === undefined
			) {
...
describe('Encounter Template Migration (Phase 0)', () => {
	describe('Fixture 1: Simple Template', () => {
		...
		it('should infer roles correctly', () => {
			const migrated = migrateOldTemplate(simpleTemplateFixture);
			const boss = migrated.variants[0].participants.find(
				(p) => p.tag === 'Boss'
			);
			const lackey = migrated.variants[0].participants.find(
				(p) => p.tag === 'Lackey'
			);
	...
	describe('Fixture 3: Hazard Template', () => {
		...
		it('should correctly assign hazard roles', () => {
			const migrated = migrateOldTemplate(hazardTemplateFixture);
			const hazards = migrated.variants[0].participants.filter(
				(p) => p.type === 'hazard'
			);
		...
		it('should mix creatures and hazards correctly', () => {
			const migrated = migrateOldTemplate(hazardTemplateFixture);
			const creatures = migrated.variants[0].participants.filter(
				(p) => p.type === 'creature'
			);
			const hazards = migrated.variants[0].participants.filter(
				(p) => p.type === 'hazard'
	...
	describe('Data Preservation Checklist', () => {
		...
		it('should always have defaultVariantId pointing to valid variant', () => {
			allFixtures.forEach((fixture) => {
				...
				const hasDefault = migrated.variants.some(
					(v) => v.id === migrated.defaultVariantId
				);
		...
		it('should ensure no participant data loss in simple case', () => {
			...
			const expectedTotal = simpleTemplateFixture.participants.reduce(
				(sum, p) => sum + (p.count ?? 1),
				0
			);
