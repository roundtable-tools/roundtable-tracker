...
function migrateParticipant(oldParticipant: OldParticipant): Participant {
  ...
  switch (oldParticipant.type) {
    ...
    default:
      throw new Error(`Unknown participant type: ${(oldParticipant as any).type}`);
  }
}
 ...
 */
export function migrateOldTemplate(oldTemplate: OldStoreTemplate): EncounterTemplateData {
  ...
  const extraVariants = (oldTemplate.variants ?? []).map((variant) =>
    migrateVariant(variant, oldTemplate.partySize ?? 4, oldTemplate.participants ?? [])
  );
 ...
 */
export function migrateOldTemplates(oldTemplates: OldStoreTemplate[]): EncounterTemplateData[] {
