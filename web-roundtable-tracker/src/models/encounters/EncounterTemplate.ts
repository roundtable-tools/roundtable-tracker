...
import type { EncounterTemplateData, Participant, Event } from './encounter.types';
...
export class EncounterTemplate {
  ...
  private calculateParticipantXp(participant: Participant): number {
    const countXp = participant.relativeLevel
      .toExperience(participant.type !== 'hazard' || participant.role === 'complex')
